...
const replacePath = async () => {
  ...
  try {
    glob("static/**/*.js").then(async (files) => {
    ...
    // glob("static/**/*.js", {}, async (error, files) => {
...
const validateBEM = () =>
  gulp
    .src([`static/*.html`, `!static/popup-*.html`, `!static/responses-*.html`])
    ...
    .pipe(
      htmlValidator.analyzer({
        ignoreMessages: new RegExp(
          "^Section lacks heading. Consider using “h2”-“h6”|^Element “img” is missing required attribute “src”.|Element “source” is missing required attribute “srcset”|^Article lacks heading. Consider using “h2”-“h6” elements to add identifying headings to all articles."
        ),
      })
    )
    .pipe(
