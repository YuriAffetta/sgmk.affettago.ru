...
import { search, toggleSearch } from "../../components/header/header.js";
 ...
 * @return {void}
 */
export function setAttr(array, attr, str) {
  for (let index = 0; index < array.length; index++) {
    const element = array[index];
    if (!element.hasAttribute(attr)) {
      element.setAttribute(attr, str ? str : "false");
    }
  }
}
/**
...
/**
 * Переключает указанный класс каждому элементу в заданном массиве.
 *
 * @param {Array} array - Массив элементов, у которого будет переключен класс.
 * @param {string} className - Имя класса, которое будет переключено у каждого элемента.
 * @return {undefined} Эта функция не возвращает значения.
 */
export function toggleClasses(array, className) {
  for (var i = 0; i < array.length; i++) {
